...
class StyleGAN2Loss(Loss):
    ...
    def run_dxdy_zero(self, gen_img):
        loss=0
        for i in range(len(gen_img)):
            zeros = torch.zeros_like(gen_img[i])
            mse = torch.mean(torch.square(zeros[i] - gen_img[i]))
            loss+=mse
        loss=loss/len(gen_img)
        return loss

    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, conditioning_params, gain, cur_nimg, overfit_z, overfit_real_img):
        assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth']
        real_img = overfit_real_img
        ...
        #import PIL.Image
        #realimg = (real_img['image'].permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        #PIL.Image.fromarray(realimg[0].cpu().numpy(), 'RGB').save(f'hi2.png')
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_img, _gen_ws = self.run_G(overfit_z, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
                loss = criterion(gen_img['image'], real_img['image'])
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'l1':
            ...
            ws = self.G.mapping(overfit_z, c_gen_conditioning, update_emas=False)
            if self.style_mixing_prob > 0:
                with torch.autograd.profiler.record_function('style_mixing'):
                    cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
                    cutoff = torch.where(torch.rand([], device=ws.device) < self.style_mixing_prob, cutoff, torch.full_like(cutoff, ws.shape[1]))
            ...
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), overfit_z, ws, update_emas=False)['sigma']
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(overfit_z, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                #import PIL.Image
                #realimg = (real_img['image'].permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
                #PIL.Image.fromarray(realimg[0].cpu().numpy(), 'RGB').save(f'hi.png')
                real_img_tmp_image = real_img['image'].detach().requires_grad_(phase in ['Dreg', 'Dboth'])
                real_img_tmp_image_raw = real_img['image_raw'].detach().requires_grad_(phase in ['Dreg', 'Dboth'])
                real_img_tmp = {'image': real_img_tmp_image, 'image_raw': real_img_tmp_image_raw}

                #real_logits = self.run_D(real_img_tmp, conditioning_params, blur_sigma=blur_sigma) # adaptation 위해서 변경
